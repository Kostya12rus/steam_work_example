...
class AppIDContent(ft.Container):
    ...
    def __on_click_delete_button(self, e):
        sql_manager.appdetails_del(self.app_details)
        callback_manager.trigger(EventName.ON_APP_ID_REMOVED, self.app_details)
...
class AppIDsPageContent(ft.Column):
    ...
    @staticmethod
    def get_appdetails(appid: str) -> AppDetails | None:
        url = f'https://store.steampowered.com/api/appdetails?appids={appid}'
        req = requests.get(url)
        if not req.ok: return None
        req_json = req.json()
        if not req_json: return None
        app_details = req_json.get(str(appid), {})
        if not app_details and not app_details.get('success', False): return None
        return AppDetails(app_details.get('data', {}))
